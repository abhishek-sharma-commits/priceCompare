      ...
      <div className="flex flex-wrap justify-center gap-3 max-w-xl mx-auto px-4">
        {SUGGESTIONS.map((term, i) => (
          <motion.button
            ...
            className="cursor-target group flex items-center gap-2 px-5 py-2 rounded-full border border-white/5 bg-white/[0.03] backdrop-blur-xl text-[12px] font-mono text-white/70 hover:text-white hover:border-white/30 hover:bg-white/10 transition-all duration-300"
          >
            {/* Tiny Indicator Dot */}
            <div className="w-1 h-1 rounded-full bg-white/20 group-hover:bg-[#C2F84F] transition-colors" />
            {term}
          </motion.button>
        ))}
      </div>
